...
# chi''(u) + (2/u + 1/2(1+y) )chi'(u) + chi(u) = -24f_v(0)/u^2 integral from 0 to u of
...
def f2(chi, chi_prime, u, idx):
    dchi_primedu = (
        ...
        - Q**2 * chi / (1 + u)
        - 24 * fv0 / u**2 / (1 + u) *
        scipy.integrate.simps(chi_prime * func(u), x)
    )
    return dchi_primedu
...
def func(ran):
    ...
    return arr
    '''     
    return -np.sin(ran) / ran**3 - 3 * np.cos(ran) / ran**4 + 3*np.sin(ran) / ran**5
...
ax1.plot(x, (chi_prime / Chi_prime)**2, label="|chi'/ chi'_0|^2", color="orange")
'''
ax1.vlines(
    [y_GW_Reenter],
    -.6,
...
'''
ran = np.linspace(-100, 100, 1000)
# ax2.plot(ran, -np.sin(ran) / ran**3 - 3 *
#        np.cos(ran) / ran**4 + 3*np.sin(ran) / ran**5)
